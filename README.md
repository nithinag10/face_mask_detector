...
There are some complications when to comes to deploying in the real world. In this app, face recognition works fine which is built on resnet. But when it comes to face detection, haar_cascade from cv2 won't do the job greatly. If we use any advanced face detection technique we can definitely put it into real world usage. This can be implemented in CCTV in public places where this  application a has job.
